...
export class NvidiaNimService implements AIProvider {
  ...
  async generate(request: AIRequest): Promise<AIResponse> {
    ...
    const model = process.env.NVIDIA_NIM_MODEL || process.env.NVIDIA_MODEL || 'meta/llama-3.3-70b-instruct';
    ...
    if (!res.ok) {
      ...
      throw new Error(`NVIDIA NIM API error (HTTP ${res.status}): ${errorDetails}`);
    }
